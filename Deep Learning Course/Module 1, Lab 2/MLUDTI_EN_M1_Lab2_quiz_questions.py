...
question_1 = Quiz(question={
            'question': "Which option would you use to create a single-layer neural network with 3 output classes for 16x16 images?",
            'options': [
                "nn.Sequential(\n\
                nn.Flatten(),\n\
                nn.Linear(256, 1)\n\
...
question_2 = Quiz(question={
            'question': "Which option would you use for the train_net() function to use 20 epochs and a learning rate of 0.5?",
            'options': [
                "train_net(mlp, training_loader, validation_loader, num_epochs=20, lr=0.5)", 
                "train_net(mlp, training_loader, validation_loader, num_epochs=0.5, learning_rate=20)", 
                "train_net(mlp, training_loader, validation_loader, num_epochs=20, learning_rate=0.5)"
            ],
            'correctIndex': 2,
        })

question_3 = Quiz(question={
            'question': "Which option would you use to specify a dropout layer that drops 70 percent of the nodes?",
            'options': [
                "nn.Dropout(70%)", 
                "nn.Dropout(0.7)", 
                "nn.Dropout(1-0.7)"
